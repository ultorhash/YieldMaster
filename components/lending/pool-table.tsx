...
const PoolRow = memo(function PoolRow({ pool, onSelect }: PoolRowProps) {
  const exploit = EXPLOITED_PROTOCOLS[pool.protocol]

  return (
    <tr className="border-b border-border/50 hover:bg-muted/20 transition-colors odd:bg-transparent even:bg-muted/10">
      <td className="p-4">
      ...
      <td className="p-4">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2" style={{ backgroundColor: PROTOCOL_COLORS[pool.protocol as keyof typeof PROTOCOL_COLORS] || '#888' }} />
          <span className="text-sm text-foreground">{pool.protocol}</span>
        </div>
      </td>
      <td className="p-4">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2" style={{ backgroundColor: CHAIN_COLORS[pool.chain as keyof typeof CHAIN_COLORS] || '#888' }} />
      ...
      <td className="p-4 text-right">
        <div className="flex items-center justify-end gap-1.5">
          ...
          {pool.rewardApy > 0 && (
            <span className="font-mono text-[11px] text-violet-500 flex items-center gap-1" title="Rewards">
      ...
      <td className="p-4 text-center">
        <span
          ...
          style={{ backgroundColor: RISK_COLORS[pool.riskRating] + '20', color: RISK_COLORS[pool.riskRating] }}
        >
          {pool.riskRating}
        </span>
      </td>
      <td className="p-4 text-center">
        <span className={`text-xs font-medium ${pool.sigma < 0.1 ? 'text-primary' : pool.sigma < 0.25 ? 'text-yellow-500' : 'text-destructive'}`}>
      ...
      <td className="p-4 text-center">
        <div className="flex items-center justify-center gap-1">
          ...
          <span title={exploit ? `${exploit.type === 'logic' ? 'Logic' : 'Oracle'} exploit` : 'No Exploits'}>
            <Bug className={`h-4 w-4 ${exploit?.type === 'logic' ? 'text-destructive' : exploit?.type === 'oracle' ? 'text-yellow-500' : 'text-muted-foreground/30'}`} />
          </span>
          <button onClick={() => onSelect(pool)} className="p-1 ml-1 cursor-pointer group" title="View details">
...
const SortTh = memo(function SortTh({ label, sortKeyName, align = 'left', sortKey, sortDirection, onSort }: SortThProps) {
  return (
    <th className="p-4 cursor-pointer select-none" onClick={() => onSort(sortKeyName)}>
    ...
    <>
      <div className="bg-card border border-border overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-border">
          <h2 className="text-sm font-medium text-foreground uppercase tracking-wider">Lending Pools</h2>
          <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
            <div className="flex items-center gap-1.5"><Gift className="h-4 w-4 text-violet-500" /><span>Rewards</span></div>
            <div className="flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-primary" /><span>Audited</span></div>
            <div className="flex items-center gap-1.5"><Shield className="h-4 w-4 text-chart-4" /><span>Insurance</span></div>
            <div className="flex items-center gap-1.5"><Bug className="h-4 w-4 text-yellow-500" /><Bug className="h-4 w-4 text-destructive" /><span>Exploit</span></div>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <SortTh label="Asset" sortKeyName="asset" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
                <SortTh label="Protocol" sortKeyName="protocol" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
                <SortTh label="Chain" sortKeyName="chain" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
                <SortTh label="Supply APY" sortKeyName="supplyApy" align="right" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
                <SortTh label="TVL" sortKeyName="tvl" align="right" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
                <SortTh label="Risk" sortKeyName="riskRating" align="center" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
                <SortTh label="Volatility" sortKeyName="sigma" align="center" sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
      ...
      <PoolDetailModal pool={selectedPool} onClose={handleClose} />
    </>
  )
}
