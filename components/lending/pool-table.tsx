  ...
  const sortedPools = [...pools].sort((a, b) => {
    ...
    switch (sortKey) {
      ...
      case 'utilization':
        ...
        break
      case 'riskRating':
        comparison = riskOrder[a.riskRating] - riskOrder[b.riskRating]
        break
    }
  ...
  const SortHeader = ({ label, sortKeyName }: { label: string; sortKeyName: SortKey }) => (
    ...
    >
      ...
      <span className="flex flex-col">
        <ChevronUp 
          className={`h-3 w-3 -mb-1 ${sortKey === sortKeyName && sortDirection === 'asc' ? 'text-primary' : 'text-muted-foreground/40'}`} 
        />
        <ChevronDown 
          className={`h-3 w-3 ${sortKey === sortKeyName && sortDirection === 'desc' ? 'text-primary' : 'text-muted-foreground/40'}`} 
        />
      </span>
    </button>
  )
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th className="text-left p-4">
                  <SortHeader label="Protocol" sortKeyName="protocol" />
                </th>
                <th className="text-left p-4">
                  <SortHeader label="Chain" sortKeyName="chain" />
                </th>
                <th className="text-right p-4">
                  <SortHeader label="Supply APY" sortKeyName="supplyApy" />
                </th>
                <th className="text-right p-4">
                  <SortHeader label="Borrow APY" sortKeyName="borrowApy" />
                </th>
                <th className="text-right p-4">
                  <SortHeader label="TVL" sortKeyName="tvl" />
                </th>
                <th className="text-right p-4">
                  <SortHeader label="Utilization" sortKeyName="utilization" />
                </th>
                <th className="text-center p-4">
                  <SortHeader label="Risk" sortKeyName="riskRating" />
                </th>
                <th className="text-center p-4">
                  ...
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-1">
                      <ShieldCheck 
                        ...
                        title={pool.audited ? 'Audited' : 'Not Audited'} 
                      />
                      <Shield 
                        className={`h-4 w-4 ${pool.insuranceCoverage ? 'text-chart-3' : 'text-muted-foreground/30'}`} 
                        title={pool.insuranceCoverage ? 'Insurance Coverage' : 'No Insurance'} 
                      />
                      <Bug 
                        className={`h-4 w-4 ${pool.hadExploit ? 'text-destructive cursor-help' : 'text-muted-foreground/30'}`}
                        title={pool.hadExploit ? (pool.exploitDetails || 'Protocol had exploit in the past') : 'No Past Exploits'} 
                      />
                      <button
